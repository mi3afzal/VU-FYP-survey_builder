		...
		return this.each(function () {
			...
			if (opts.load_url) {
				$.getJSON(opts.load_url, function(json) {
					if(json.form_id != '') form_db_id = json.form_id;
					fromJson(json.form_structure);
					if(json.responses>0) alert('This survey have been conducted '+json.responses+' time. So any changes will save as new survey.');
				});
			}
			...
			var save = function () {
					if (opts.save_url) {
						$.ajax({
							...
							}) + "&form_id="+form_db_id+"&form_title="+$('#survey_title').val()+"&expired_at="+$('#expiration_date').val()+
							"&status="+$('#status').val(),
							success: function (responce){
								alert(responce.message);
								if(typeof form_db_id == 'undefined'){
									form_db_id = responce.id;
									//alert(form_db_id);
								}
							}
						});
