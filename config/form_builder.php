...
class Formbuilder {
	...
	public function process(){
		...
		$success = true;
		if(is_array($error) and count($error) > 0) $success = false;
	...
	protected function loadCheckboxGroup($field)
	{
		...
		if(isset($field['values']) && is_array($field['values'])){
			...
			foreach($field['values'] as $item){
				...
				$item = (array)$item;
				$item_id = $this->elemId($item['value'], $field['title']);
				
				// set the default checked value
				$checked = $item['baseline'] == 'true' ? true : false;

				// load post value
				//$val = $this->getPostValue($item_id);
				//if($val != false and $val != '') $checked = true;

				// if checked, set html
				$checked = $checked ? ' checked="checked"' : '';

				$checkbox = '<span class="row clearfix"><input type="checkbox" id="%s" name="%s" value="%s" %s /><label for="%s">%s</label></span>'."\n";
				$html .= sprintf($checkbox, $item_id, $item_id, $item['value'], $checked, $item_id, $item['value']);
			}
			$html .= sprintf('</span>') . "\n";
		}
		$html .= '</li>' . "\n";
	...
	protected function getPostValue($key){
		$return_val = false;
		if(is_array($_SESSION['page_post_data']) and count($_SESSION['page_post_data']) > 0)
		{
			if(array_key_exists($key, $_SESSION['page_post_data'])) $return_val = $_SESSION['page_post_data'][$key];
		}
		else if(is_array($_POST) and count($_POST) > 0)
		{
			if(array_key_exists($key, $_POST)) $return_val = $_POST[$key];
		}
		return $return_val;
